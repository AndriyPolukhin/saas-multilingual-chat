...
const InviteUser = ({ chatId }: { chatId: string }) => {
	...
	const onSubmit = async (values: z.infer<typeof formSchema>) => {
		...
		// We need to get the users current chats to check if theyre about to exced the PRO plan
		...
		if (!isPro && noOfUsersInChat >= 2) {
			toast({
				...
				description:
					'You have exceeded the limit of users in a single chat for the FREE plan. Please upgraded to PRO to continue adding users to chats!',
				variant: 'destructive',
				action: (
					<ToastAction
						altText='Upgrade'
